...
def runner(path, width, output):
    ...
    f = open('/'.join([output, file_name + ".txt"]),'w')
    f.write(image)
    f.close()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        width = sys.argv[2]
        output = sys.argv[3]
        runner(path, int(width), output)
    else:
        print("Usage: ./asciify.py <image-path> <desired-width> <output-location>")
